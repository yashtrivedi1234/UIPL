      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex justify-between items-end mb-10">
          <div className="max-w-xl">
            ...
            <p className="text-slate-500 mt-4 text-sm leading-relaxed">
              Discover our portfolio of ongoing and completed masterpieces
              across Lucknow's most prestigious sectors.
            </p>
          </div>
          <button
            onClick={() => navigate("/projects")}
            className="text-[#8d4f00] font-black flex items-center gap-2 hover:translate-x-1 duration-300 text-sm"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-5 h-auto md:h-[800px]">
          {projects.map(({ title, tag, tagColor, span, img }) => (
            <div
              key={title}
              className={`${span} group relative overflow-hidden rounded-2xl shadow-xl`}
            >
              <img
                src={img}
                alt={title}
              ...
              <div className="absolute bottom-7 left-7 text-white">
                <span
                  className={`font-black tracking-widest text-xs uppercase ${tagColor}`}
                >
                  {tag}
                </span>
